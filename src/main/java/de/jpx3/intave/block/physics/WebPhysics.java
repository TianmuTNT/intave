...
final class WebPhysics implements BlockPhysic {
  ...
  public Motion entityInside(User user, SimulationEnvironment environment, BlockPosition location, Position from, Motion motion, boolean insideBlockOrTooFast) {
    ...
    if (user.protocolVersion() >= VER_1_20_5) {
      boolean weaving = !environment.isInVehicle() && Effects.weavingEffectActive(user.player());
      environment.setMotionMultiplier(weaving ? new Vector(0.5D, 0.25D, 0.5D) : new Vector(0.25D, 0.05F, 0.25D));
    }
